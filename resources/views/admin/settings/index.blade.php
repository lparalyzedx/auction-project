    ...
    <div class="pf-top">
        ...
        <div class="pf-identity">
            <div class="pf-avatar-wrap">
                <div class="pf-avatar-outer" style="background:linear-gradient(135deg,#7c3aed 0%,#4f46e5 100%);display:flex;align-items:center;justify-content:center;">
...
@push('scripts')
<script>
function switchSTab(key, btn) {
    document.querySelectorAll('.pf-ptab').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    document.querySelectorAll('.s-panel').forEach(p => p.classList.toggle('s-active', p.id === 'sc-' + key));
}

function execCmd(cmd, val) { document.execCommand(cmd, false, val || null); }
function syncEditor(editorId, hiddenId) { document.getElementById(hiddenId).value = document.getElementById(editorId).innerHTML; }

function charCount(id, cntId, max) {
    document.getElementById(cntId).textContent = document.getElementById(id).value.length + '/' + max;
}

document.getElementById('site_logo')?.addEventListener('change', function () {
    if (!this.files?.[0]) return;
    const r = new FileReader();
    r.onload = e => { document.getElementById('logoPreview').innerHTML = `<img src="${e.target.result}" alt="Logo" style="width:100%;height:100%;object-fit:contain;">`; };
...
function submitActiveForm() {
    ...
    const hidden = panel.querySelector('input[type=hidden][name$="_text"]');
...
function testMail() {
    Swal.fire({
        title: 'Test E-postası', input: 'email', inputPlaceholder: 'Gönderilecek e-posta adresi',
        icon: 'question', showCancelButton: true, confirmButtonText: 'Gönder', cancelButtonText: 'Vazgeç',
        heightAuto: false, didOpen: () => document.body.classList.remove('swal2-height-auto')
    }).then(r => {
        ...
        fetch('{{ route("admin.settings.test-mail") }}', {
            ...
            Swal.fire({ icon: data.success ? 'success' : 'error', title: data.success ? 'Gönderildi!' : 'Hata!', text: data.message, heightAuto: false, didOpen: () => document.body.classList.remove('swal2-height-auto') });
        });
    });
}

document.querySelectorAll('.danger-cache-btn').forEach(btn => {
    btn.addEventListener('click', function () {
        const form = this.closest('form');
        Swal.fire({
            title: 'Emin misin?', text: 'Tüm önbellek verileri silinecek.', icon: 'warning',
            showCancelButton: true, confirmButtonText: 'Evet, temizle', cancelButtonText: 'Vazgeç',
            reverseButtons: true, heightAuto: false, didOpen: () => document.body.classList.remove('swal2-height-auto')
